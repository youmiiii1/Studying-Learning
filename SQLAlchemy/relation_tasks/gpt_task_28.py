...
class Student(Base):
    ...
    courses: Mapped[list[str]] = association_proxy("courses_rel", "title")
...
class Course(Base):
    ...
    student_names: Mapped[list[str]] = association_proxy("students", "name")

def get_student_profile(student_id):
    with Session(engine) as session:
        student = session.get(Student, student_id)
        return student.profile.email
...
def get_student_instructors(student_id):
    with Session(engine) as session:
        student = session.get(Student, student_id)
        return [course.instructor.name for course in student.courses_rel]
